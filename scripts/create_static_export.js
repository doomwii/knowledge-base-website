...
fs.writeFileSync(staticChapterPath, staticChapterContent);
console.log('已创建静态章节页面');

// 6. 创建静态管理员登录页面
const staticAdminLoginPath = path.join(__dirname, '..', 'src', 'pages', 'admin', 'login.tsx');
const staticAdminLoginDir = path.dirname(staticAdminLoginPath);

// 确保目录存在
if (!fs.existsSync(staticAdminLoginDir)) {
  fs.mkdirSync(staticAdminLoginDir, { recursive: true });
}

const staticAdminLoginContent = `import React, { useState } from 'react';
import Link from 'next/link';
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        {success ? (
          <div className="rounded-md bg-green-50 p-4">
            <div className="flex">
              <div className="ml-3">
                <h3 className="text-sm font-medium text-green-800">登录成功</h3>
...
fs.writeFileSync(staticAdminLoginPath, staticAdminLoginContent);
console.log('已创建静态管理员登录页面');

// 7. 创建静态管理后台仪表盘页面
const staticAdminDashboardPath = path.join(__dirname, '..', 'src', 'pages', 'admin', 'dashboard.tsx');

const staticAdminDashboardContent = `import React from 'react';
import Link from 'next/link';
...
fs.writeFileSync(staticAdminDashboardPath, staticAdminDashboardContent);
console.log('已创建静态管理后台仪表盘页面');

// 8. 创建一个简单的全局样式文件
const globalCssPath = path.join(__dirname, '..', 'src', 'styles', 'globals.css');
...
console.log('已创建全局样式文件');

// 9. 更新package.json，添加静态导出脚本
...
console.log('已更新package.json，添加静态导出脚本');

console.log('静态导出版本创建完成！请提交并推送更改，然后等待Vercel重新部署。');
